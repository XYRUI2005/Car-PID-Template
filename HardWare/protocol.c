  ...
  while(1)
  {
    ...
    switch (cmd_type)
    {
      ...
      case SET_P_I_D_CMD:
      {
        ...
        /*要调那个PID就取消哪个注释*/
//        set_p_i_d(&pid_speed1, p_temp, i_temp, d_temp);    // 设置那个改那个，只能通过通道观察
//        set_p_i_d(&pid_location1, p_temp, i_temp, d_temp);
//        set_p_i_d(&pid_speed2, p_temp, i_temp, d_temp); 
//        set_p_i_d(&pid_location2, p_temp, i_temp, d_temp);    // 设置那个改那个，只能通过通道观察
//        set_p_i_d(&pid_turn_angle,p_temp, i_temp, d_temp);
//				set_p_i_d(&pid_line,p_temp, i_temp, d_temp);
      ...
      case SET_TARGET_CMD:
      {
        ...
        set_point = actual_temp;
          ...
          /*要调哪个PID就取消哪个注释*/
//        set_pid_target(&pid_speed1, set_point);   
//        set_pid_target(&pid_location1, set_point);
//        set_pid_target(&pid_speed2, set_point);   
//        set_pid_target(&pid_location2, set_point);
//        set_pid_target(&pid_turn_angle, set_point);
//				set_pid_target(&pid_line, set_point);
      }
      break;
...
//        __HAL_TIM_SET_AUTORELOAD(&htim6,temp);               // 设置定时器周期1~1000ms
