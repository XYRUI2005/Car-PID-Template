...
void pid_param_init(void)
{
    ...
    /* 角度环相关初始化参数 */
    pid_turn_angle.target_val=0.0;
    pid_turn_angle.actual_val=0.0;
    pid_turn_angle.err=0.0;
    pid_turn_angle.err_last=0.0;
    pid_turn_angle.integral=0.0;
    
    pid_turn_angle.Kp=1.1;
    pid_turn_angle.Ki=0.0;
    pid_turn_angle.Kd=0.0;
        
    /* 转向速度相关初始化参数 */
    pid_line.target_val=0.0;
    pid_line.actual_val=0.0;
    pid_line.err=0.0;
...
float turn_angle_pid_realize(_pid *pid, float actual_val)
{
    ...
    if((pid->err >= 354.0f) || (pid->err <= -354.0f)) //修改这里的值可以解决陀螺仪边界值突变问题
    {
      pid->err = 0;
      pid->integral = 0;
    }
